...
import type { Route } from './+types/cron.inbox-poll';

/**
 * POST /api/cron/inbox-poll
 *
 * Pulls new signing emails from the WorkHub inbox into the signature queue. Wire
 * to your cron service (every 1–5 minutes). Auth: `Authorization: Bearer
 * $NEXT_PRIVATE_CRON_SECRET`.
 */
export const action = async ({ request }: Route.ActionArgs) => {
  ...
  const result = await pollAllOrgInboxes();
